# -*- coding: utf-8 -*-
"""
Created on Sat Jul 28 10:42:08 2018
...
class optStruct:
    def __init__(self,dataMatIn, classLabels, C, toler, kTup):  # 存储各类参数
        ...
        self.alphas = np.mat(np.zeros((self.m,1)))#alphas初始化为0
        self.b = 0 #初始设为0
...
def innerL(i, oS): #输入参数i和所有参数数据
    ...
    if ((oS.labelMat[i]*Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or ((oS.labelMat[i]*Ei > oS.tol) and (oS.alphas[i] > 0)): #检验这行数据是否符合KKT条件 参考《统计学习方法》p128公式7.111-113
        ...
        if (oS.labelMat[i] != oS.labelMat[j]): #判断同号一号，以下代码的公式参考《统计学习方法》p126？？？
            #？？？为什么不直接用alphaIold,alphaJold
        ...
        updateEk(oS, j)
        if (np.abs(oS.alphas[j] - alphaJold) < oS.tol): #alpha变化大小阀值（自己设定）
...
def smoP(dataMatIn, classLabels, C, toler, maxIter,kTup=('lin', 0)): #输入参数：数据特征，数据类别，参数C，阀值toler，最大迭代次数，核函数（默认线性核）
    ...
    while (iter < maxIter) and ((alphaPairsChanged > 0) or (entireSet)):
        ...
        if entireSet:
            ...
            nonBoundIs = np.nonzero((oS.alphas.A > 0) * (oS.alphas.A < C))[0]#.A的意思是mat转化为array
...
def testRbf(data_train,data_test):
    ...
    svInd=np.nonzero(alphas)[0]  #选取不为0数据的行数（也就是支持向量），svInd返回的非零元素下标值
...
#主程序
def main():
    filename_traindata=r'C:\Users\liks\liks\10IT\SVM source code\train.txt'
    filename_testdata=r'C:\Users\liks\liks\10IT\SVM source code\test.txt'
